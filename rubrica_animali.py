    ...
    try:
        ...
        if len(nominativo.get()) == 0: # Controllo se la entry nominativo sia a 0 
            ...
            if os.path.isdir("Rubrica/" + animale.get()) == 0: # Verifico che la sottocartella esista
                ...
                logCapture(log)
                print() # WorkAround per il problema di quando aggiungi un pdf o non lo dichiari e aggiungi il contatto ti restituisce il contenuto 
            else:
                try: ######################################################
